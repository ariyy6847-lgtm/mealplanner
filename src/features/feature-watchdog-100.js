// Watchdog handling for MealPlanner.
const { EventEmitter } = require('node:events');

class WatchdogHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'watchdog' + ' payload');
 return { 'watchdog': String(payload).slice(0, 200) };
 }
...
module.exports = { WatchdogHandler, run: (payload, opts) => new WatchdogHandler().run(payload, opts) };